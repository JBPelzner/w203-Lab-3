...
The variable of interest we identify for measuring a important environmental factor is `density`, representing the the number of people per square mile in a county. We hypothesize that higher population density leads to an increase in crime rate for two main reasons:
...
2. In counties with high population density, people are likey to be living in more confined spaces and are probably more easily irritated, thus making them more likely to act out through ciminal behavior.
...
```{r}
vars.2 <- c('crmrte', 'prbarr', 'density', 'polpc', 'urban', 'wfed')
...
Observations & 90 & 90 \\ 
R$^{2}$ & 0.554 & 0.616 \\ 
Adjusted R$^{2}$ & 0.543 & 0.593 \\ 
Residual Std. Error & 0.013 (df = 87) & 0.012 (df = 84) \\ 
\hline 
\hline \\[-1.8ex] 
